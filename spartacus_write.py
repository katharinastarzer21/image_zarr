...
def write_data(var):
    year = 2025
    fill_value = -999
    artifact_path = f"data/downloaded_2025/SPARTACUS2-DAILY_{var}_{year}.nc"
    ...
    # Zarr-Store und -Gruppe öffnen (ohne Anführungszeichen bei Variable!)
    store = zarr.storage.LocalStore(zarr_path)
    group = zarr.group(store=store)  # Anhängen erlauben
    zarr.consolidate_metadata(store)

    print(group['TX'])

    # X/Y-Länge prüfen (zur Sicherheit)
    x_extent = group["x"][:]
    y_extent = group["y"][:]

    # Rohdaten laden und aufbereiten
